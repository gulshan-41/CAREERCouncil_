...
export function CategoriesProvider({ children }) {
    ...
    const fetchCategoryDetails = useCallback(
        async (catID) => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Failed to fetch data for ${catID}: ${response.status}`);
                }
                const data = await response.json();
                ...
                console.error(`Category details error for ${catID}:`, err.message);
                setDetailsError((prev) => ({ ...prev, [catID]: err.message }));
            } finally {
                setDetailsLoading((prev) => ({ ...prev, [catID]: false }));
            }
    ...
    const fetchCourseDetails = async (courseID) => {
        ...
        try {
            ...
            console.error(`Course details error for ${courseID}:`, error.message);
            setCourseError((prev) => ({ ...prev, [courseID]: error.message }));
        } finally {
            setCourseLoading(false);
        }
